...
void plot_graph()
{
    ...
    HPDF_REAL x_graph1, y_graph1;
    HPDF_REAL x_graph2, y_graph2;
    int i;
    ...
    HPDF_REAL points2[] = {5, 2, 4, 2, 7, 6, 8, 9, 6, 4, 3, 2, 1, 6, 4, 2, 9, 4, 3, 2,
                          9};
    ...
    HPDF_Page_MoveTo(page, x_graph1, y_graph1);
    ...
    {
        x_graph1 += 10;
        y_graph1 = points[i] * 20 + mov_up;
        HPDF_Page_LineTo(page, x_graph1, y_graph1);
    }
    HPDF_Page_Stroke(page);
    
    HPDF_Page_SetRGBStroke(page, 1, 0, 0);//red
    HPDF_Page_MoveTo(page, x_graph2, y_graph2);
    for (i = 0; i < sizeof(points2) / sizeof(HPDF_REAL); i++)
    {
        x_graph2 += 10;
        y_graph2 = points2[i] * 20 + mov_up;
        HPDF_Page_LineTo(page, x_graph2, y_graph2);
    }
...
void print_table()
{
    ...
    HPDF_Page_SetLineWidth(page, 1);
    HPDF_Page_SetRGBStroke(page, 1, 0, 1);
    /* Draw table */
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 7; j++)
